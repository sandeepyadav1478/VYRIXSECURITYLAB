...
from sherlock_package.result import QueryStatus, QueryResult
from sherlock_package.notify import QueryNotifyPrint
from sherlock_package.sites  import SitesInformation
...
def spruce(username, notext=None, updatecheck=False):


    moc_out = ['FOR USER: '+' '.join([str(elem) for elem in username])+' FOLLOWING LINKS ARE AVAILABLE ==>>','']
    if not bool(username):
        raise Exception("Didn't got username to perform action.")
    ...
    for username in args.username:
        ...
        # Modified output source
        for website_name in results:
                dictionary = results[website_name]
                if dictionary.get("status").status == QueryStatus.CLAIMED:
                    moc_out.append(dictionary["url_user"])
